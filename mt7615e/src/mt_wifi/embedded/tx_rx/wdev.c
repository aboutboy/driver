...
VOID BssInfoArgumentLinker(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	if(wdev->channel > 14)
		HTPhyMode = pAd->CommonCfg.MCastPhyMode_5G;
	else
		HTPhyMode = pAd->CommonCfg.MCastPhyMode;
	//printk("%s: McastPhyMode.MODE=%d, MCS=%d, BW=%d\n", __FUNCTION__, HTPhyMode.field.MODE, HTPhyMode.field.MCS, HTPhyMode.field.BW);
#else
