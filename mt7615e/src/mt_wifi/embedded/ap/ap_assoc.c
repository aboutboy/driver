		...
		*/
			...
			ie_list->vht_cap_len)
		{
			...
			*/
			...
			dot11_vht_mcs_to_internal_mcs(pAd, &ie_list->vht_cap, &pEntry->MaxHTPhyMode);
...
SendAssocResponse:
	...
	if ((ie_list->ht_cap_len > 0) && WMODE_CAP_N(wdev->PhyMode))
	{
        ...
        if (HcIsBfCapSupport(wdev) == FALSE)
        {
            ...
            mt_WrapSetETxBFCap(pAd, &HtCapabilityRsp.TxBFCap);
...
/*		if (IS_FT_RSN_STA(pEntry) && (FtInfo.FtIeInfo.Len != 0)) */
		...
		if (TRUE) /*CFG_TODO*/
        {
...
#endif /* RT_CFG80211_P2P_CONCURRENT_DEVICE */
			{
				...
				CFG80211OS_NewSta(pEntry->wdev->if_dev, ie_list->Addr2, (PUCHAR)Elem->Msg, Elem->MsgLen);
