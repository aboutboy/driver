...
#ifdef CON_WPS
	if (pwdev->func_type == OMAC_TYPE_APCLI)
        {
		USHORT ifIndex=pwdev->func_idx;
		pWpsCtrl = &pAd->ApCfg.ApCliTab[pwdev->func_idx].WscControl;
	        if (pWpsCtrl->conWscStatus != CON_WPS_STATUS_DISABLED)
	        {
	              MlmeEnqueue(pAd, AP_SYNC_STATE_MACHINE, APMT2_MLME_SCAN_COMPLETE, 0, NULL, ifIndex );
