	...
	IN	RTMP_STRING *arg)
{
	...
	{
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("==>Set_RadioOn_Proc (%s) equal to current state, ignore!!!\n", 
			radio? "ON":"OFF"));
		return TRUE;
	}
...
#ifdef SINGLE_SKU_V2
	pAd->CommonCfg.SKUenable = TxPowerSKUEn;
#endif /* SINGLE_SKU_V2 */
...
INT SetPercentageCtrl(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
    ...
    pAd->CommonCfg.PERCENTAGEenable = TxPowerPercentEn;

    return TxPowerPercentCtrl(pAd, TxPowerPercentEn);
}
...
INT SetBfBackoffCtrl(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
    ...
    /* Update Profile Info for Power Percentage */
    pAd->CommonCfg.BFBACKOFFenable = TxBFBackoffEn;
    
    return TxPowerBfBackoffCtrl(pAd, TxBFBackoffEn);
}
