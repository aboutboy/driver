...
VOID BuildBgndScanChList(RTMP_ADAPTER *pAd)
{
	...
	{	/* 2.4G only support BW20 background scan */
		for (channel_idx = 0; channel_idx < pAd->ChannelListNum; channel_idx++)	
		{
			...
			pAd->BgndScanCtrl.BgndScanChList[ChListNum].SkipChannel = BackgroundScanSkipChannelCheck(pAd, pAd->ChannelList[channel_idx].Channel);
			ChListNum++;
		}	
	}
...
VOID GroupChListInsert(PRTMP_ADAPTER pAd, PBGND_SCAN_SUPP_CH_LIST pSource)
{
	...
	__FUNCTION__, pAd->BgndScanCtrl.GroupChListNum, GroupChList->CenChannel, GroupChList->BestCtrlChannel,	GroupChList->Max_PCCA_Time, GroupChList->SkipGroup));
}
                ...
                IN PRTMP_ADAPTER pAd)
{
	...
	pAd->BgndScanCtrl.IPIIdleTime = TotalIPI;
        ...
        //				__FUNCTION__, lv0, lv1, lv2, lv3, lv4, lv5, lv6, lv7, lv8, lv9, lv10, TotalIPI, Noisy, ChBusyTime, MyTxAirTime, MyRxAirTime));
