...
BOOLEAN get_class_by_channel(UCHAR *ChannelSet, UCHAR Channel)
{
	int index = 0;
...
UCHAR get_regulatory_class(RTMP_ADAPTER *pAd, UCHAR Channel, UCHAR PhyMode, struct wifi_dev *wdev)
{
	...
	if (WMODE_CAP_AC(PhyMode))
	{
	    do
	    {
			if (reg_class_vht_fcc[i].spacing >= bw)
	        {
				/* find  channel_set */
				if (get_class_by_channel(reg_class_vht_fcc[i].channel_set, Channel) ||
					(get_class_by_channel(reg_class_vht_fcc[i].center_freq, vht_cent_ch_freq(Channel, cfg_vht_bw))))
				{
					regclass = reg_class_vht_fcc[i].reg_class;
					break;
				}
			}
			i++;
		} while (reg_class_vht_fcc[i].reg_class != 0);
	}
	else
#endif /* DOT11_VHT_AC */		
	{
		...
		do
	    {
			if (reg_class_fcc[i].spacing >= bw)
	        {
				/* find  channel_set */
				if (get_class_by_channel(reg_class_fcc[i].channel_set, Channel))
				{
					regclass = reg_class_fcc[i].reg_class;
					break;
				}
			}
			i++;
		} while (reg_class_fcc[i].reg_class != 0);
	}
    //ASSERT(regclass);
	return regclass;
...
{
	int i=0, Channel = 0, Type = 0, IsAc = 0, OpClass = 0;
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		(":%s: Type = %d Channel = %d IsAc = %d\n", __FUNCTION__, Type, Channel, IsAc));
	...
	else
	{
		...
		if (IsAc)
		{
			do
		    {
				...
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					("%-8d%-8d", reg_class_vht_fcc[i].reg_class, reg_class_vht_fcc[i].spacing));

				while (reg_class_vht_fcc[i].channel_set[j] != 0)
				{
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
						("%-2d,", reg_class_vht_fcc[i].channel_set[j]));
					j++;
				}

				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,("/"));

				while (reg_class_vht_fcc[i].center_freq[k] != 0)
				{
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
						("%-2d,", reg_class_vht_fcc[i].center_freq[k]));
					k++;
				}
...
#endif /* DOT11_VHT_AC */			
		{
			do
		    {
				...
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					("%-8d%-8d", reg_class_fcc[i].reg_class, reg_class_fcc[i].spacing));

				while (reg_class_fcc[i].channel_set[j] != 0)
				{
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					("%-2d,", reg_class_fcc[i].channel_set[j]));
					j++;
				}
