	...
	for (HashIdx = 0; HashIdx < FT_R1KH_ENTRY_HASH_TABLE_SIZE; HashIdx++)
	{
		...
		while (pEntry != NULL)
		{
				...
				&& (pEntry->KeyLifeTime--) == 0)
			{
				...
				MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_ERROR, 
					("%s: PMKCache timeout. Kick out the station and delete FT_R1khEntry!\n",
							__FUNCTION__));
				/*
	...
	OUT PFT_INFO 			pFtInfo_out)
{
	...
	if ((result = WPAValidateRSNIE(sec_group, sec_config, 
		pFtInfo_in->RSN_IE, pFtInfo_in->RSNIE_Len)) != MLME_SUCCESS) {				
		return result;
	}
	...
	for (Loop = 0; Loop < MAX_MBSSID_NUM(pAd); Loop++)
	{
		...
		*/
		...
		if(RTMPGetKeyParameter(tok_str, tmpbuf, FT_MDID_LEN + 1, pBuffer, FALSE))
		{
			...
			{
				NdisMoveMemory(pAd->ApCfg.MBSSID[Loop].wdev.FtCfg.FtMdId, tmpbuf, FT_MDID_LEN);
				MTWF_LOG(DBG_CAT_PROTO, CATPROTO_FT, DBG_LVL_TRACE, ("%s::FtMdid(%d)=%c%c\n", __FUNCTION__, Loop,
											pAd->ApCfg.MBSSID[Loop].wdev.FtCfg.FtMdId[0],
											pAd->ApCfg.MBSSID[Loop].wdev.FtCfg.FtMdId[1]));
			}
			else
			{
