...
VOID MtAsicAddRemoveKeyByDriver(struct _RTMP_ADAPTER *pAd, ASIC_SEC_INFO *pInfo);
BOOLEAN MtAsicCheckDMAIdle(struct _RTMP_ADAPTER *pAd,UINT8 Dir);
...
#if  defined(MT7615)
VOID PciResetPDMAToMakeSurePreFetchIndexCorrect(struct _RTMP_ADAPTER *pAd);
...
INT32 MtAsicRxHeaderTaranBLCtl(struct _RTMP_ADAPTER *pAd, UINT32 Index, BOOLEAN En, UINT32 EthType);
/* SMAC Start */
INT MtSmacAsicDisableBeacon(struct _RTMP_ADAPTER *pAd, VOID *wdev_void);
INT MtSmacAsicEnableBeacon(struct _RTMP_ADAPTER *pAd, VOID *wdev_void, UCHAR NumOfBcns);
