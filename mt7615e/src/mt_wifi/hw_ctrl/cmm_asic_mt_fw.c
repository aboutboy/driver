	...
	IN struct _ASIC_SEC_INFO *pInfo)
{
	...
	if (IS_CIPHER_WEP(pInfo->Cipher))
	{
        ...
        {
            rWtblSecurityKey.ucAlgorithmId = CIPHER_SUIT_BIP;
        }
#ifdef WAPI_SUPPORT
        else if (IS_CIPHER_WPI_SMS4(pInfo->Cipher))
        {
            rWtblSecurityKey.ucAlgorithmId = CIPHER_SUIT_WPI;
        }
#endif /* WAPI_SUPPORT */
        else
        {
	...
	OUT UCHAR *pTxTsc)
{
	...
	{
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): invalid apidx(%d)\n",
					__FUNCTION__, apidx));
		return;
	}
...
#if defined(MT7615) || defined(MT7622)
	val1 = WtblDwQuery(pAd, Wcid, 1, 9);
	val2 = WtblDwQuery(pAd, Wcid, 1, 10);
#endif /* defined(MT7615) || defined(MT7622) */

#if defined(MT736) || defined(MT7637)
	val1 = WtblDwQuery(pAd, Wcid, 2, 0);
	val2 = WtblDwQuery(pAd, Wcid, 2, 1);
#endif /* MT7636 || MT7637 */

	*pTxTsc 	= val1 & 0xff;
	*(pTxTsc+1) = (val1 >> 8) & 0xff;
	*(pTxTsc+2) = (val1 >> 16) & 0xff;
	*(pTxTsc+3) = (val1 >> 24) & 0xff;

	*(pTxTsc+4) = val2 & 0xff;
	*(pTxTsc+5) = (val2 >> 8) & 0xff;
...
#ifdef MT_PS
VOID MtSetIgnorePsmByFw(
	RTMP_ADAPTER *pAd,
	MAC_TABLE_ENTRY *pEntry,
	UCHAR value)
{
	CMD_WTBL_PEER_PS_T		CmdWtblPeerPs = {0};
	...
	CmdExtWtblUpdate(pAd, (UINT8)pEntry->wcid, SET_WTBL, (PUCHAR)&CmdWtblPeerPs, sizeof(CMD_WTBL_PEER_PS_T));
}
#endif /* MT_PS */
...
INT MtAsicSetRDGByFw(RTMP_ADAPTER *pAd, MT_RDG_CTRL_T *Rdg)
{
    ...
    fw_rdg.u4TxOP = cpu2le32(Rdg->Txop);
