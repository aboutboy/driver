	...
	UINT						Data)
{
	...
	if(Data == tdls_insert_entry)
	{
		...
		if (pMacEntry)
		{
			...
			//MTWF_LOG(DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_ERROR,("=====>  %s() line: %d  supRateLen: %d ExtRateLen:%d Htlen:%d\n"	, __FUNCTION__,__LINE__,pAd->StaActive.SupRateLen,pAd->StaActive.ExtRateLen,HtLen));  //Kyle Debug
			...
			{
				//MTWF_LOG(DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_ERROR,("=====>  %s() line: %d  TDLS setting Encryption\n", __FUNCTION__,__LINE__));  //Kyle Debug
			...
			WifiSysTdlsPeerLinkDown(pAd,pMacEntry);
	...
	IN PVOID SystemSpecific3)
{
	...
	*/
	...
	if (pTDLS->FlgIsWaitingUapsdTraRsp == TRUE)
	{
		...
		/*
		cfg_tdls_build_frame(pAd,pTDLS->MacAddr,1,TDLS_ACTION_CODE_AUTO_TEARDOWN,0,NULL,0,FALSE,0,25);
		mdelay(100);
		if(pTDLS->EntryValid == TRUE)
		{
...
INT cfg_tdls_build_frame(PRTMP_ADAPTER pAd,u8 *peer,u8 dialog_token,u8 action_code,u16 status_code,const u8 *extra_ies,size_t extra_ies_len,BOOLEAN send_by_tdls_link,u8 tdls_entry_link_index,u8 reason_code)
{
		...
		|| action_code == WLAN_TDLS_SETUP_RESPONSE)
	{
		...
		if (pAd->StaCfg[0].wdev.UapsdInfo.bAPSDCapable)
		...
		&& os_equal_mem(TDLS_LLC_SNAP_WITH_CATEGORY, pRxBlk->pData, LENGTH_802_1_H + 2))
	{
		...
		if(TDLSType == TDLS_ACTION_CODE_SETUP_REQUEST|| TDLSType == TDLS_ACTION_CODE_SETUP_RESPONSE)
		{
			...
			else
			{
				...
				MTWF_LOG(DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_OFF,("%s() - TDLSEntry[%d] Initiator = %d\n", __FUNCTION__,tdls_entry_index,pAd->StaCfg[0].wpa_supplicant_info.CFG_Tdls_info.TDLSEntry[tdls_entry_index].bInitiator));
			...
			while ((Length + 2 + pEid->Len) <= RemainLen)
			{
				switch(pEid->Eid)
				{
					...
					case IE_VENDOR_SPECIFIC:
						...
						,__FUNCTION__, pAd->StaCfg[0].wpa_supplicant_info.CFG_Tdls_info.TDLSEntry[tdls_entry_index].QosCapability, PRINT_MAC(peerMAC)));
						break;
					case IE_HT_CAP:
		...
		else if(TDLSType == TDLS_ACTION_CODE_CHANNEL_SWITCH_REQUEST)
		{
			...
			RTEnqueueInternalCmd(pAd, CMDTHREAD_TDLS_SEND_CH_SW_SETUP, &TdlsCHSWParam, sizeof(CFG_TDLS_CHSW_PARAM));					mdelay(1);
			cfg_tdls_chsw_resp(pAd,peerMAC,pCfgTdls->ChSwitchTime,pCfgTdls->ChSwitchTimeout,0);
		}
		else if(TDLSType == TDLS_ACTION_CODE_CHANNEL_SWITCH_RESPONSE)
		{
			...
			else if(pAd->StaCfg[0].wpa_supplicant_info.CFG_Tdls_info.IsentCHSW == TRUE && pAd->StaCfg[0].wpa_supplicant_info.CFG_Tdls_info.bChannelSwitchInitiator == TRUE)
			{
				...
				/*if(StatusCode != 0)
				{
					DBGPRINT(RT_DEBUG_OFF,("PeerTdlsChannelSwitchRsp status code : %d , stop channel switch!! \n",StatusCode));
					pCfgTdls->bDoingPeriodChannelSwitch = FALSE;
					return;
				}
				else
				...
				RTEnqueueInternalCmd(pAd, CMDTHREAD_TDLS_SEND_CH_SW_SETUP, &TdlsCHSWParam, sizeof(CFG_TDLS_CHSW_PARAM));
				/*DBGPRINT(RT_DEBUG_ERROR,("START CH SW!! Initiator:%d Base: %d Target:%d BW:%d SW Time:%d SW Timeout:%d OffChannelStayTime:%d"
				"peerMAC:(%02X:%02X:%02X:%02X:%02X:%02X)\n"
				,pCfgTdls->bChannelSwitchInitiator,pCfgTdls->BaseChannel,pCfgTdls->TargetOffChannel,pCfgTdls->TargetOffChannelBW
				,pCfgTdls->ChSwitchTime,pCfgTdls->ChSwitchTimeout,pCfgTdls->OffChannelStayTime,PRINT_MAC(peerMAC)));
...
)
{
	...
	MTWF_LOG(DBG_CAT_PROTO, CATPROTO_TDLS, DBG_LVL_ERROR,("%s(): CHSW Resp Status:%d , SwitchTime:%d , SwitchTimeout:%d \n"
			,__FUNCTION__,reason_code,ch_sw_time,ch_sw_timeout));
