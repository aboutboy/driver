...
static INT MtAsicSetTmacInfoTemplate(RTMP_ADAPTER *pAd)
{
	...
	dw5->TxSFmt = 0;
	dw5->TxS2Host= 0; // Disable TxS
	dw5->BarSsnCtrl = 0; //HW
#if defined(CONFIG_STA_SUPPORT) && defined(CONFIG_PM_BIT_HW_MODE)
	dw5->PwrMgmt= TMI_PM_BIT_CFG_BY_HW; // HW
#else
...
VOID MtAsicUpdateRxWCIDTableByDriver(RTMP_ADAPTER *pAd,MT_WCID_TABLE_INFO_T WtblInfo)
{
	...
	}else
	{
		...
		switch (WtblInfo.WcidType)
		{
            ...
            case MT_WCID_TYPE_REPEATER:
				...
				break;
			default:
			 	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Unknown entry type(%d)\
				do not support header translation\n", __FUNCTION__, WtblInfo.WcidType));
				break;
		}
#endif
	...
	HW_IO_WRITE32(pAd, tb_entry.wtbl_addr[0], dw0->word);
        ...
        MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(%d):Write WTBL 1 Addr:0x%x, Value:0x%x\n",
				__FUNCTION__, WtblInfo.Wcid, tb_entry.wtbl_addr[0], dw0->word));
        MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(%d):Write WTBL 1 Addr:0x%x, Value:0x%x\n",
				__FUNCTION__, WtblInfo.Wcid,  tb_entry.wtbl_addr[0] + 4, dw1->word));
        MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(%d):Write WTBL 1 Addr:0x%x, Value:0x%x\n",
				__FUNCTION__, WtblInfo.Wcid, tb_entry.wtbl_addr[0] + 8, dw2->word));
	...
	HW_IO_WRITE32(pAd, tb_entry.wtbl_addr[0] + 12, dw3->word);
        MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(%d):Write WTBL 1 Addr:0x%x, Value:0x%x\n",
				__FUNCTION__, WtblInfo.Wcid, tb_entry.wtbl_addr[0] + 12, dw3->word));

	HW_IO_WRITE32(pAd, tb_entry.wtbl_addr[0] + 16, dw4->word);
        MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(%d):Write WTBL 1 Addr:0x%x, Value:0x%x\n",
				__FUNCTION__, WtblInfo.Wcid, tb_entry.wtbl_addr[0] + 16, dw4->word));
...
#ifdef MT_PS
        if (IS_ENTRY_MCAST(&pAd->MacTab.Content[cnt]))
			dw3->field.i_psm=1;
			dw3->field.du_i_psm=1;
		}
#endif
...
VOID MtAsicAddRemoveKeyByDriver (
    IN PRTMP_ADAPTER pAd,
    ...
    {   /* Add Key */
...
#ifdef WAPI_SUPPORT
        else if (IS_CIPHER_WPI_SMS4(pInfo->Cipher))
        {
            addr = KeyId*32 + addr;
            dw2->field.cipher_suit = CIPHER_SUIT_WPI;
        ...
        else
        {
            MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(%d): Not support Cipher[0x%x] \n",
                __FUNCTION__, __LINE__, pInfo->Cipher));
            return;
        }
...
INT MtAsicSetWPDMA(RTMP_ADAPTER *pAd, INT32 TxRx, BOOLEAN enable, UINT8 WPDMABurstSIZE)
{
	...
	case PDMA_TX:
		if (enable == TRUE)
		{
			...
			GloCfg.field.WPDMABurstSIZE = WPDMABurstSIZE;
		} else {
...
UINT32 MtAsicGetRxStat(RTMP_ADAPTER *pAd, UINT type)
{
	...
	switch (type) {
		...
		{
            PHY_IO_READ32(pAd,CR_PHYMUX_5, &value);//::TODO CMD_GET_MACFCSERRORCNT_COUNT
			value &= 0xFF0FFFFF;
...
INT MtAsicStartContinousTx(RTMP_ADAPTER *pAd, UINT32 PhyMode, UINT32 BW, UINT32 PriCh, UINT32 Mcs, UINT32 WFSel)
{
	...
	PHY_IO_WRITE32(pAd,CR_PHYMUX_10,0x09400000);//3
#else
	/* load the tag_pwr */
...
INT MTAsicSetTMR(RTMP_ADAPTER *pAd,UCHAR enable)
{
	...
	switch (enable)
	{
		case 1://initialiter
		{
			...
			MAC_IO_READ32(pAd,RMAC_TMR_PA, &value);//::TODO CMD_GET_MACFCSERRORCNT_COUNT
			value = value | BIT31;
			value = value & ~BIT30;
...
INT32 MtAsicDMASchedulerInit(RTMP_ADAPTER *pAd, MT_DMASCH_CTRL_T DmaSchCtrl)
{
	...
	MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): DMA Scheduler Mode=%d(%s)\n",
...
INT MtSmacAsicEnableBeacon(RTMP_ADAPTER *pAd, VOID *wdev_void, UCHAR NumOfBcns)
{
    /*
        for MT_MAC, NumOfBcns is useless,
        only used for function call type unify.
    */
...
INT MtSmacAsicDisableBeacon(RTMP_ADAPTER *pAd, VOID *wdev_void)
{
    struct wifi_dev *wdev = (struct wifi_dev *)wdev_void;
    BCN_BUF_STRUC *bcn_info = &wdev->bcn_buf;
    BOOLEAN OperationResult = TRUE;
