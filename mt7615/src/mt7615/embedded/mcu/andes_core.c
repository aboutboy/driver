...
VOID AndesInitCmdMsg(struct cmd_msg *msg, CMD_ATTRIBUTE attr)
{
        ...
        // TODO: shiang-MT7615, recovery me after issue fixed!
        MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_INFO,
                ("%s:: mcu_dest(%d), cmd_type(0x%x), ExtCmdType(0x%x)\n",
                __FUNCTION__, attr.mcu_dest, attr.type, attr.ext_type));
...
static NDIS_SPIN_LOCK *AndesGetSpinLock(struct MCU_CTRL *ctl, DL_LIST *list)
{
	...
	else
	{
                MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_ERROR, 
                                        ("list pointer = %p\n", list));
		MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_ERROR, 
                        ("txq = %p, rxq = %p, ackq = %p, kickq = %p,"
                                        " tx_doneq = %p, rx_doneq = %p\n",
...
INT32 AndesSendCmdMsg(PRTMP_ADAPTER ad, struct cmd_msg *msg)
{
	...
	if (IsStopingPdma(&ad->ErrRecoveryCtl)) {
		...
		if (!RTMP_TEST_FLAG(ad, fRTMP_ADAPTER_MCU_SEND_IN_BAND_CMD)) {
                        MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
                                ("%s: Could not send in band command due to diable" 
                                "fRTMP_ADAPTER_MCU_SEND_IN_BAND_CMD\n", __FUNCTION__));
		} else if (RTMP_TEST_FLAG(ad, fRTMP_ADAPTER_NIC_NOT_EXIST)) {
			MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s: Could not send in band command due to "
				"fRTMP_ADAPTER_NIC_NOT_EXIST\n", __FUNCTION__));
		} else if (RTMP_TEST_FLAG(ad, fRTMP_ADAPTER_SUSPEND)) {
			MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s: Could not send in band command "
				"due to fRTMP_ADAPTER_SUSPEND\n", __FUNCTION__));
		}
...
retransmit:
	...
	if (is_cmd_need_wait) {
		...
		if(!IsComplete) {
			...
			MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_ERROR, 
        			("FWCmdTimeout: command (%x), "
        				"ext_cmd_type (%x), seq(%d), timeout(%dms)\n", 
			...
			if (msg->state == wait_cmd_out_and_ack) {
				...
				{			
					/* timeout process */
					if ((msg->attr.type == EXT_CID) &&
						(msg->attr.ext_type == EXT_CMD_PS_RETRIEVE_START))
					{
						MtPsSendToken(ad, msg->wcid);
					}
					if ((msg->attr.type == EXT_CID) &&
						(msg->attr.ext_type == EXT_CMD_PWR_SAVING))
					{
