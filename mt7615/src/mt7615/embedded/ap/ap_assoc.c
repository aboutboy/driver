    ...
    )
{
	...
	if (StatusCode == MLME_SUCCESS)
	{
        ...
        if (pAd->CommonCfg.LinkTestSupport)
        {
            if (pAd->fgTxSpur)
            {
                if (!pAd->Detect_20_CBW)
                {
                    /* Enter BW20 when only one STA with 11n and BW20 configuration */
                    if ((pEntry->MaxHTPhyMode.field.BW == BW_20) && (pAd->MacTab.Size == 1))
                    {
                        /* Obtain 2G Info */
                        pAd->ucOriChannel_2G = HcGetChannelByRf(pAd, RFIC_24GHZ);
                        pAd->ucOriCenterChannel_2G = HcGetCentralChByRf(pAd, RFIC_24GHZ);
                        pAd->ucOriCBW_2G = HcGetBwByRf(pAd, RFIC_24GHZ);

                        /* Obtain 5G Info */
                        pAd->ucOriChannel_5G = HcGetChannelByRf(pAd, RFIC_5GHZ);
                        pAd->ucOriCenterChannel_5G = HcGetCentralChByRf(pAd, RFIC_5GHZ);
                        pAd->ucOriCBW_5G = HcGetBwByRf(pAd, RFIC_5GHZ);

                        MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("pAd->ucOriCBW_2G = %d, pAd->ucOriChannel_2G = %d, pAd->ucOriCenterChannel_2G = %d \n",
                                                                                pAd->ucOriCBW_2G, pAd->ucOriChannel_2G, pAd->ucOriCenterChannel_2G));

                        MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("pAd->ucOriCBW_5G = %d, pAd->ucOriChannel_5G = %d, pAd->ucOriCenterChannel_5G = %d \n",
                                                                                pAd->ucOriCBW_5G, pAd->ucOriChannel_5G, pAd->ucOriCenterChannel_5G));
                        
                        /* Channel switch for change to CBW20 */
                        AsicSetChannel(pAd, pAd->ucOriChannel_2G, BW_20, EXTCHA_NONE, TRUE);
                        AsicSetChannel(pAd, pAd->ucOriChannel_5G, BW_20, EXTCHA_NONE, TRUE);

                        /* update State Stautus */
                        pAd->Detect_20_CBW = TRUE;
                    }
                }
            }
        }
	...
	IN BOOLEAN isReassoc)
{
	...
	if (pEntry->HTC_AAD_OM_Freeze == TRUE)
	{
		/*
			should not happen here, error check for debug
		*/
		MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("@@@ %s(): ERROR! HTC_AAD_OM_Freeze is in Asso. stage, (wcid=%u)\n",
		__FUNCTION__, pEntry->wcid));
		pEntry->HTC_AAD_OM_Freeze = FALSE;
	}
...
SendAssocResponse:
	...
	if ((ie_list->ht_cap_len > 0) && WMODE_CAP_N(wdev->PhyMode))
	{
		...
		if (wlan_config_get_ht_gi(wdev) == GI_400 && HtCapabilityRsp.HtCapInfo.ChannelWidth == HT_BW_40) {
