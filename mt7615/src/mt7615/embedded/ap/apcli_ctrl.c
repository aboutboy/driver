...
static VOID ApCliTrialConnectTimeoutAction(PRTMP_ADAPTER pAd, MLME_QUEUE_ELEM *pElem)
{
	...
	if ((pAd->CommonCfg.BBPCurrentBW != pAd->hw_cfg.bbp_bw) ||
		(pApCliEntry->TrialCh != HcGetChannelByRf(pAd,(HcGetBandByWdev(&pApCliEntry->wdev)+1)))) {
		/* switch channel to original channel */
		ch = HcGetCentralChByRf(pAd,(HcGetBandByWdev(&pApCliEntry->wdev)+1));
...
static VOID ApCliTrialPhase2TimeoutAction(PRTMP_ADAPTER pAd, MLME_QUEUE_ELEM *pElem)
{
	...
	if ((BW_20 != HcGetBw(pAd,&pApCliEntry->wdev))||
		(pApCliEntry->TrialCh != HcGetChannelByRf(pAd,(HcGetBandByWdev(&pApCliEntry->wdev)+1)))) {
		/* TBD disable beacon? */
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, 
				 ("%s[%d]Change bw %s to 20M,ch %d to %d\n\r",
				  __func__,__LINE__,
				  bw_str[HcGetBw(pAd,&pApCliEntry->wdev)],
				  wdev->channel,pApCliEntry->TrialCh));
		/* switch channel to trial channel */
		pAd->hw_cfg.bbp_bw = HcGetBw(pAd,&pApCliEntry->wdev);
		HcBbpSetBwByChannel(pAd,BW_20,pApCliEntry->TrialCh);
		...
		(pWpsCtrl->bWscTrigger == TRUE))
	{
		...
		if (pWpsCtrl->WscMode == 1) /* PIN */
		{
			...
			else
			{
				...
				UCHAR channel = wdev->channel, RootApChannel = pAd->ScanTab.BssEntry[bss_idx].Channel;
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	{
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE, ("(%s) Probe respond fail.\n", __FUNCTION__));
		*pCurrState = APCLI_CTRL_DISCONNECTED;
#ifdef APCLI_AUTO_CONNECT_SUPPORT
		if ((pAd->ApCfg.ApCliAutoConnectRunning[apcli_ifIndex] == TRUE)
#ifdef MAC_REPEATER_SUPPORT
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	pApCliEntry = &pAd->ApCfg.ApCliTab[ifIndex];
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE, ("(%s) Auth Req Timeout.\n", __FUNCTION__));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE, ("(%s) Assoc Req Timeout.\n", __FUNCTION__));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE, ("(%s) MLME Request disconnect.\n", __FUNCTION__));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE, ("(%s) Peer DeAssoc Req.\n", __FUNCTION__));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (!(pApCliEntry->TrialCh)) {
		...
		if ((BW_20 != HcGetBw(pAd,&pApCliEntry->wdev))||
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("(%s) pAd->CommonCfg.CentralChannel:%d\n", __func__, pAd->CommonCfg.CentralChannel));
