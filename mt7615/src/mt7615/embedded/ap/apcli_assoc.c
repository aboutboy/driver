	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	{
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE, ("APCLI_ASSOC - Block Auth request durning WPA block period!\n"));
		...
		{
			{
				...
				{ /* Todo by Eddy: It's not good code */
					...
					/* Need to fill the pSecConfig->PmfCfg to let WPAMakeRSNIE() generate correct RSNIE*/
					{
							...
							pSecConfig->PmfCfg.UseSHA256Connect = FALSE;
							...
							if ((pSecConfig->PmfCfg.MFPC) && (RsnCap.field.MFPC))
							{
								...
								if ((apcli_entry->MlmeAux.IsSupportSHA256KeyDerivation) || (RsnCap.field.MFPR))
									pSecConfig->PmfCfg.UseSHA256Connect = TRUE;
							}
					}
#endif /* DOT11W_PMF_SUPPORT */
					
					WPAMakeRSNIE(apcli_entry->wdev.wdev_type, pSecConfig);
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (NStatus != NDIS_STATUS_SUCCESS)
	{
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE, ("APCLI_ASSOC - ApCliMlmeDisassocReqAction() allocate memory failed\n"));
		*pCurrState = APCLI_ASSOC_IDLE;
	...
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE, ("APCLI_ASSOC - Send DISASSOC request [BSSID::%02x:%02x:%02x:%02x:%02x:%02x] \n",
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	{
		MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE, ("APCLI_ASSOC - ApCliPeerAssocRspAction() sanity check fail\n"));
	}
	...
	IN MLME_QUEUE_ELEM *Elem)
{
    ...
    {
        MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE, ("APCLI_ASSOC - ApCliPeerDisassocAction() sanity check fail\n"));
    }
