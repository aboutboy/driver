...
/*==================================================== */
	...
	BOOLEAN HTC_AAD_OM_Force; //when reach the threshold, force set the WTBL.DW2.AAD_OM to 1
	UINT32 HTC_AAD_OM_CountDown; //settling time (1 count 1 second) for start count HTC_ICVErrCnt
	ULONG HTC_AAD_OM_Freeze; //Treat the entry's AAD_OM setting is correct now
...
typedef struct _MAC_TABLE {
	...
	/*
		Be care in mgmt_entrytb.c  MacTableDeleteEntry() will NdisZeroMemory(&pAd->MacTab.Size, sizeof(MAC_TABLE)-offsetof(MAC_TABLE, Size));
...
struct _RTMP_ADAPTER {
...
#ifdef NR_PD_DETECTION
    UCHAR    ucOriChannel_2G;
    UCHAR    ucOriCBW_2G;
    UCHAR    ucOriCenterChannel_2G;

    UCHAR    ucOriChannel_5G;
    UCHAR    ucOriCBW_5G;
    UCHAR    ucOriCenterChannel_5G;

    INT32    c4NrRssiTh;
    INT32    c4ChgTestPathTh;
    INT32    c4MaxInRssiTh;

    BOOLEAN  Detect_20_CBW;
    BOOLEAN  Detect_1R_PD;
    UINT8    NO_ACK_COUNT;
    UINT8    NR_Timeout_Count;
    UINT32   TempRxCount;
    UINT32   NullFrameCount;
    UINT32   NullFrameFailCount;
    UINT8    RSSITh;
    UINT8    RSSITh_HighPower;
    UINT8    RSSITh_LowPower;
    INT32    LargePowerTh;
    UINT8    RxCountTh;
    UINT8    TimeOutTh;
    BOOLEAN  EntryOneSTADebug;
    BOOLEAN  EntryVHTModeDebug;
    BOOLEAN  Entry1NSSDebug;
    BOOLEAN  EntryRSSIDebug;
    BOOLEAN  DepartOneSTADebug;
    BOOLEAN  DepartPERDebug;
    BOOLEAN  DepartTIMEOUTDebug;
    BOOLEAN  DepartNULLACKDebug;
    BOOLEAN  NRPDDebugLog;

    BOOLEAN  fgNRPDDetectEnable;
    BOOLEAN  fgDefaultRSSIAccess;
    ...
    UINT8    Backto4Tcount;
    UINT8    Backto4TThres;
    UINT32   BackUpCSDConfig;

    UINT8    ucACIConfidenceCnt;
    UINT8    ucACIConfidenceCntTh;
    UINT8    ucMaxInConfidenceCnt;
    UINT8    ucMAxInConfidenceCntTh;

    BOOLEAN  fgWIFIInitDone;
    BOOLEAN  fgChannelSwitchDone;

    /* manual command control function enable/disable flag */
    BOOLEAN  fgTxSpur;
    BOOLEAN  fgNrFloating;
    BOOLEAN  fgACI;
#endif /* NR_PD_DETECTION */
...
#ifdef CFG_SDIO_RX_THREAD
	/*for raising task priority avoiding context switch too much*/
	NDIS_SPIN_LOCK RxThreadLock;
#endif
#endif
...
/*=======Common=========== */
	...
	*/
	...
	RTMP_STRING MC_BinFileName[256];
	RTMP_STRING MC_CalBinFileName[256];
...
BOOLEAN is_udp_packet(RTMP_ADAPTER *pAd, PNDIS_PACKET pPacket);
...
INT TxPowerBoundCtrl(PRTMP_ADAPTER pAd, UCHAR PwrUppBound);
INT TxPowerRfTxAnt(PRTMP_ADAPTER pAd, UCHAR TxAntCtrlEn, UCHAR WIFI_0_En, UCHAR WIFI_1_En, UCHAR WIFI_2_En, UCHAR WIFI_3_En);
INT TxPowerShowInfo(PRTMP_ADAPTER pAd, UCHAR TxPowerInfoEn);
