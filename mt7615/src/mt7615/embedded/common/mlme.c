...
//CFG MCC
    ...
    if (pAd->CommonCfg.LinkTestSupport)
    {
        ...
        {
            if (pAd->fgNRPDDetectEnable)
            {
                if (pAd->fgDefaultRSSIAccess)
                {
                    /* Configure RCPI Calculation Method to reference Both Response Frame and Data Frame (0x820F9808[23] = 0x1) */
                    MtCmdLinkTestRCPICtrl(pAd, TRUE);

                    /* Update RCPI Access Flag */
                    pAd->fgDefaultRSSIAccess = FALSE;
                }
        ...
        else
        {
            ...
            if (!pAd->fgDefaultRSSIAccess)
            {
                ...
                {
                    /* Restore RCPI Config */
                    MtCmdLinkTestRCPICtrl(pAd, FALSE);
