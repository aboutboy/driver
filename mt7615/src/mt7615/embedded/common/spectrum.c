...
CHAR RTMP_GetTxPwr(RTMP_ADAPTER *pAd, HTTRANSMIT_SETTING HTTxMode, UCHAR Channel)
{
	UINT32 Value;
	INT Idx;
	UINT8 PhyMode;
...
static VOID PeerMeasureReportAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	if (pMeasureReportInfo == NULL)
	{
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s unable to alloc memory for measure report buffer (size=%zu).\n", __FUNCTION__, sizeof(MEASURE_RPI_REPORT)));
		return;
	}
