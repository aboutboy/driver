...
UINT32   WscGetAuthMode(
    IN  USHORT authFlag);

UINT32   WscGetWepStatus(
    IN  USHORT encryFlag);
    ...
    IN PVOID SystemSpecific3)
{
	...
	if (pOpposAd)
	{
        ...
        else
        {
			...
			if (pAd->ApCfg.ConWpsApCliMode == CON_WPS_APCLI_BAND_2G)	
				WscStop(pAd, TRUE, &pAd->ApCfg.ApCliTab[BSS1].WscControl);
	...
	for(index = 0; index < MAX_APCLI_NUM; index++)
        	pAd->ApCfg.ApCliTab[index].WscControl.conWscStatus = CON_WPS_STATUS_DISABLED;
    ...
    IN MLME_QUEUE_ELEM  *Elem)
{
	...
	if (pWpsCtrl == NULL)
	{
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR, ("%s: pWpsCtrl == NULL!\n", __FUNCTION__));
		return;
	}
...
#ifdef CON_WPS
	{
		    ...
		    {   
				...
				pApCliWpsCtrl = &pAd->ApCfg.ApCliTab[pEntry->func_tb_idx].WscControl;
		        else
	...
	IN	MLME_QUEUE_ELEM	*Elem)
{
	...
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscEAPAction: Addr: %02x:%02x:%02x:%02x:%02x:%02x, MsgType: 0x%02X, bUPnPMsg: %s\n",
	...
	if (CurOpMode == AP_MODE)
	{
		if (!bUPnPMsg)
		{
			if (pEntry)
			{
				...
				{
					MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscEAPAction: Unknow apidex(=%d).\n", pEntry->func_tb_idx));
					MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("<----- WscEAPAction\n"));
					return;
				}
				else
				{
					...
					MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscEAPAction: apidex=%d.\n", pEntry->func_tb_idx));
				}
			}
			else
			{
				MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscEAPAction: pEntry is NULL.\n"));
				MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("<----- WscEAPAction\n"));
				return;
			}
#ifdef APCLI_SUPPORT
	...
	if (pEntry && IS_ENTRY_CLIENT(pEntry))
	{
		if ((MsgType == WSC_MSG_EAP_REG_RSP_ID) || (MsgType == WSC_MSG_EAP_ENR_RSP_ID))
		{
			...
			{
				MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscEAPAction: Already receive EAP_RSP(Identitry) from this STA, ignore it.\n"));
				MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("<----- WscEAPAction\n"));
				return;
			}
			else
			...
			if (CurOpMode == AP_MODE)
			{
				...
				if (pWscControl->conWscStatus != CON_WPS_STATUS_DISABLED)
				{
				   	if  (pAdapter->ApCfg.ConWpsApCliMode == CON_WPS_APCLI_BAND_AUTO)
						...
						UINT opposIfaceIdx = !multi_inf_get_idx(pAdapter);
						...
						if (pOpposAd->ApCfg.ConWpsApCliStatus == TRUE)
						{
							MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF, ("%s another Band WPS ready\n", pAdapter->net_dev->name));
							...
							if (pOpposAd->ApCfg.ConWpsMonitorTimerRunning)
							{
								...
								//BandSelection
								if (pAdapter->ApCfg.ConWpsApCliMode == CON_WPS_APCLI_BAND_2G)
								...
								if (pAdapter->ApCfg.ConWpsApCliMode == CON_WPS_APCLI_BAND_2G)
									pAdapter->ApCfg.ApCliTab[1].Enable = FALSE;
								else
									pAdapter->ApCfg.ApCliTab[0].Enable = FALSE;

								ApCliIfDown(pAdapter);
								RTMPSetTimer(&pAdapter->ApCfg.ConWpsApCliBandMonitorTimer, CON_WPS_APCLIENT_MONITOR_TIME);
								pAdapter->ApCfg.ConWpsMonitorTimerRunning = TRUE;								
							}
	...
	else if (MsgType == WSC_MSG_M1)
	{
		...
		*/
		if ((pWscControl->WscConfMode & WSC_REGISTRAR) != 0)
		{
			...
			{
				MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("UPnP Registrar is working now, ignore EAP M1.\n"));
				goto out;
			}
			else
			{
	...
	IN  PWSC_CTRL       pWscControl)
{
    ...
    MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscEapEnrolleeAction Enter!\n"));
	...
	if (CurOpMode == AP_MODE)
	{
		...
		{
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("EarlyCheckFailed: pWscControl->WscActionMode=%d, Configured=%d, WscUseUPnP=%d, pEntry=%p!\n",
...
#endif /* CONFIG_AP_SUPPORT */
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("MsgType=0x%x, WscState=%d, bUPnPMsg=%d!\n", MsgType, pWscControl->WscState, bUPnPMsg));
	...
	switch (MsgType)
	{
		case WSC_MSG_EAP_RSP_ID:
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscEapEnrolleeAction : Rx Identity(ReComputePke=%d)\n", pWscControl->RegData.ReComputePke));
		case WSC_MSG_EAP_REQ_START:
			if (MsgType == WSC_MSG_EAP_REQ_START)
				MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscEapEnrolleeAction : Rx Wsc_Start(ReComputePke=%d)\n", pWscControl->RegData.ReComputePke));
		...
		case WSC_MSG_M8:
			...
			WscSendUPnPMessage(pAdapter, (pWscControl->EntryIfIdx & 0x0F), 
									...
									&pAdapter->ApCfg.MBSSID[pWscControl->EntryIfIdx].wdev.bssid[0], MAC_ADDR_LEN, 0, 0, &pAdapter->ApCfg.MBSSID[pWscControl->EntryIfIdx& 0x0F].wdev.bssid[0], AP_MODE);
#endif /* VENDOR_FEATURE6_SUPPORT */
			...
			if (pWscControl->WscState == WSC_STATE_WAIT_M1)
			{
				...
				else
				{
					...
					if (pWscControl->bTriggerByNFC)
					{
						...
						{
							MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WSC_ERROR_PUBLIC_KEY_HASH_MISMATCH  ConfigError=%d \n", pWscControl->RegData.SelfInfo.ConfigError));
							bSendM2D = TRUE;
						}
					}
#endif /* WSC_NFC_SUPPORT */
	...
	if (CurOpMode == AP_MODE)
	{
			...
			((nWscState == WSC_STATE_WAIT_DONE) || (nWscState == WSC_STATE_WAIT_ACK)))
		{
									...
									FALSE);
			{
				...
				RfIC = wmode_2_rfic(pMbss->wdev.PhyMode);				
				/*
					...
					@20160321
				*/
				RTEnqueueInternalCmd(pAd, CMDTHREAD_AP_RESTART, &RfIC, sizeof(UCHAR));
			}
...
VOID WscInitRegistrarPair(RTMP_ADAPTER *pAd, WSC_CTRL *pWscControl, UCHAR apidx)
{
	...
	if (CurOpMode == AP_MODE)
	{
		if (!RTMP_TEST_FLAG(pWscControl, 0x04)) {
			...
			sprintf(&pWscControl->RegData.SelfInfo.DeviceName[0], "%s_%d", AP_WSC_DEVICE_NAME, apidx);
		}
	}
#endif /* CONFIG_AP_SUPPORT */
	...
	IN  UCHAR		CurOpMode)
{
	...
	if (CurOpMode == AP_MODE)
	{
    		MAKE_802_3_HEADER(Header802_3,
    						  pBssid,
    						  &pAdapter->ApCfg.ApCliTab[if_idx].wdev.if_addr[0],
    						  EAPOL);
    }    
#endif /* APCLI_SUPPORT */
#endif /* CONFIG_AP_SUPPORT */
	...
	{
		/* Update WSC status */
		pAdapter->ApCfg.ApCliTab[if_idx].WscControl.WscStatus = STATUS_WSC_EAPOL_START_SENT;
		pAdapter->ApCfg.ApCliTab[if_idx].WscControl.WscState = WSC_STATE_WAIT_REQ_ID;
		if (!pAdapter->ApCfg.ApCliTab[if_idx].WscControl.EapolTimerRunning)
		{
			pAdapter->ApCfg.ApCliTab[if_idx].WscControl.EapolTimerRunning = TRUE;
			RTMPSetTimer(&pAdapter->ApCfg.ApCliTab[if_idx].WscControl.EapolTimer, WSC_EAPOL_START_TIME_OUT);
		}
	}
#endif /* APCLI_SUPPORT */
#endif /* CONFIG_AP_SUPPORT */
	...
	IN PWSC_CTRL pWscControl)
{
	...
	if (CurOpMode == AP_MODE)
	{
		...
		if(apidx < MAX_APCLI_NUM)
		{
		    MAKE_802_3_HEADER(Header802_3,
						  &pEntry->Addr[0],
						  &pAdapter->ApCfg.ApCliTab[apidx].wdev.if_addr[0],
						  EAPOL);
		}
    ...
    {
        MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscConfMode(%d) is not WSC_REGISTRAR nor WSC_ENROLLEE.\n", pWscControl->WscConfMode));
        goto out;
    }
...
/*	UCHAR 			Data[256]; */
	...
	if (CurOpMode == AP_MODE)
	{
        if ((apidx & 0x0F) < HW_BEACON_MAX_NUM)
        {  
...
/*	UCHAR 			Data[512]; */
	...
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("-----> WscBuildProbeRespIE, apidx = %x\n", apidx));
	...
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("<----- WscBuildProbeRespIE\n"));
}
	...
	if (wsc_sel_reg == 0x01)
	{
			...
			(pWscCtrl->WscState <= WSC_STATE_WAIT_M3))
		{
			...
			if (pWscPeer)
			{
				...
				MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscSelectedRegistrar --> Add this MAC to WscPeerList\n"));
			}
...
/*	UCHAR			OutMsgBuf[512];		// buffer to create message contents */
...
#ifdef APCLI_SUPPORT
	PWSC_REG_DATA	pReg = (PWSC_REG_DATA)&pAd->ApCfg.ApCliTab[ifIndex].WscControl.RegData;
#else /* APCLI_SUPPORT */
...
#else /* APCLI_SUPPORT */
	if (pAd->StaCfg[0].WscControl.WscConfMode == WSC_REGISTRAR)
		tempVal = WSC_MSGTYPE_REGISTRAR;
	else if (pAd->StaCfg[0].WscControl.WscConfMode == WSC_ENROLLEE)
		tempVal = WSC_MSGTYPE_ENROLLEE_OPEN_8021X;
	else
		tempVal = WSC_MSGTYPE_ENROLLEE_INFO_ONLY;
#endif /* !APCLI_SUPPORT */
...
#else /* APCLI_SUPPORT */
#ifdef WSC_V2_SUPPORT
	if (pAd->StaCfg[0].WscControl.WscV2Info.bEnableWpsV2)
	{
			tempVal = pAd->StaCfg[0].WscControl.WscConfigMethods;
	}
	else
#endif /* WSC_V2_SUPPORT */
	{
		tempVal = (pAd->StaCfg[0].WscControl.WscConfigMethods & 0x00FF);
	}
#endif /* !APCLI_SUPPORT */
...
#endif /* !APCLI_SUPPORT */
	{
		...
		WscGenV2Msg(&pAd->ApCfg.ApCliTab[0].WscControl,
					...
					&Len);
#else /* APCLI_SUPPORT */
		WscGenV2Msg(&pAd->StaCfg[0].WscControl,
					FALSE,
					NULL,
					0,
					&pData,
...
#ifdef CON_WPS
extern VOID APMlmeScanCompleteAction(PRTMP_ADAPTER pAd, MLME_QUEUE_ELEM *Elem);
    ...
    IN  PRTMP_ADAPTER pAd)
{
    ...
    MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("<----- WscDelWPARetryTimer\n"));
    ...
    if (pEntry)
    {
		...
		RTMPCancelTimer(&pEntry->SecConfig.StartFor2WayTimer, &Cancelled);
		RTMPCancelTimer(&pEntry->SecConfig.Handshake.MsgRetryTimer, &Cancelled);
	...
	if (pWscControl->conWscStatus == CON_WPS_STATUS_DISABLED)
        ...
        pAdListInfo[1] = 1;//RFIC_IS_5G_BAND(pOpposAd);
         ...
         MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF, ("%s pAdListInfo is5G---> [%d, %d]\n",
                __FUNCTION__, pAdListInfo[0], pAdListInfo[1]));
	...
	else
	{
        ...
        if (bFromApCli)
        {
        	if (pOpposAd)
        	{
	        	...
	        	{
	        					                pWpsCtrl = &pOpposAd->ApCfg.ApCliTab[loop].WscControl;
		...
		for (loop = 0; loop < pAd->ApCfg.ApCliNum; loop++)
		{
			if ((pWscControl->EntryIfIdx & 0x0F) == loop)
				continue;
		                ...
		                pWpsCtrl = &pAd->ApCfg.ApCliTab[loop].WscControl;
		...
		if (pOpposAd != NULL)
		{
			...
			struct wifi_dev *wdev = &pOpposAd->ApCfg.MBSSID[loop].wdev; 
	        	pWpsCtrl = &pOpposAd->ApCfg.MBSSID[loop].WscControl;
             		...
             		(pWpsCtrl->bWscTrigger == TRUE))
        	{
                	...
                	WscBuildProbeRespIE(pOpposAd, WSC_MSGTYPE_AP_WLAN_MGR, IsAPConfigured, FALSE, 0, 0,
	                                                                          loop, NULL, 0, AP_MODE);
			UpdateBeaconHandler(pOpposAd, wdev, IE_CHANGE);
				...
				struct wifi_dev *wdev = &pAd->ApCfg.MBSSID[loop].wdev; 
		        	pWpsCtrl = &pAd->ApCfg.MBSSID[loop].WscControl;
	        		IsAPConfigured = pWpsCtrl->WscConfStatus;
    ...
    IN  BOOLEAN         bEnrollee)
{
			...
			else if (pCredential->EncrType == WSC_ENCRTYPE_WEP) /* Only for WPS 2.0 */
			{
				...
				if ((pCredential->KeyIndex >= 1) && (pCredential->KeyIndex <= 4))
				{
					...
					if (WepKeyLen == 5 || WepKeyLen == 13 || WepKeyLen == 10 || WepKeyLen == 26)
					{
						...
						MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WEP: Invalid Key Length (%d)\n", pCredential->KeyLength));
				}
				else
				{
					MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("Unsupport default key index (%d)\n", WepKeyId));
#ifdef CONFIG_AP_SUPPORT
					if (CurOpMode == AP_MODE)
	...
	else
	{
		if (CurOpMode == AP_MODE)
		{
			pAd->ApCfg.MBSSID[CurApIdx].wdev.SecConfig.PairwiseKeyId = 1;
			...
			*/
			...
			{
				AtoH((RTMP_STRING *) pWscControl->WpaPsk, pAd->ApCfg.MBSSID[CurApIdx].wdev.SecConfig.PMK, 32);
			}
			else
			{
				...
				RtmpPasswordHash((RTMP_STRING *)pWscControl->WpaPsk,
							 ...
							 keyMaterial);
				NdisMoveMemory(pAd->ApCfg.MBSSID[CurApIdx].wdev.SecConfig.PMK, keyMaterial, 32);
			}
		}
	}
#endif /* CONFIG_AP_SUPPORT */
	...
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("<----- ra%d - WscWriteConfToPortCfg\n", CurApIdx));
}
	...
	IN  PWSC_CTRL       pWscControl)
{
	...
	if (CurOpMode == AP_MODE)
	{
		...
		NdisMoveMemory(pWscControl->RegData.SelfInfo.MacAddr,
	                   pAd->ApCfg.ApCliTab[apcli_idx].wdev.if_addr,
	                   MAC_ADDR_LEN);

#ifdef RTMP_TIMER_TASK_SUPPORT
		rtmp_set_channel(pAd, &pAd->ApCfg.ApCliTab[apcli_idx].wdev, pAd->ApCfg.ApCliTab[apcli_idx].MlmeAux.Channel);
#else
	...
	PUCHAR				pVar)
{
	...
	{
		if ((pAd->ApCfg.ApCliTab[apcli_idx].WscControl.conWscStatus & CON_WPS_STATUS_APCLI_RUNNING) 
			&&(pAd->ApCfg.ApCliTab[apcli_idx^1].WscControl.conWscStatus & CON_WPS_STATUS_APCLI_RUNNING)
			...
			&& pAd->ApCfg.ApCliTab[apcli_idx^1].ConWpsApCliModeScanDoneStatus != CON_WPS_APCLI_SCANDONE_STATUS_ONGOING))
			{
				...
				if (Reqwdev && HcGetBandByChannel(pAd, pInBss->Channel) != HcGetBandByWdev(Reqwdev))
					{
						if (bFound==TRUE)
							printk("FIND AP TmpInfo.Band =%d ssid=%s but skip(band=%d wdev band=%d)\n",TmpInfo.Band,TmpInfo.Ssid,HcGetBandByChannel(pAd, pInBss->Channel), HcGetBandByWdev(Reqwdev));
						bFound = FALSE;
					}
			}
	}
	...
	PUCHAR				pVar)
{
	...
	NdisZeroMemory(&TmpInfo, sizeof(UUID_BSSID_CH_INFO));
		while ((Len > 0) && (bFound == FALSE))
		{
			...
			/*if no wpsv2.0 ap found till 3 scans, enable search for wpsv1.0 ap, wpsv1.0 ap don't set selreg bit */   
	...
	IN WSC_CTRL *pWscControl)
{
	...
	if (pWscControl->WscPBCBssCount == 1)
	{
...
#ifdef CON_WPS
                        if (ApUuidBssid[0].Band == WSC_RFBAND_50GHZ)
                        {
                                RTMPMoveMemory(&TmpInfo_5G.Uuid[0], &ApUuidBssid[0].Uuid, 16);
                                TmpInfo_5G.Band = WSC_RFBAND_50GHZ;

                                MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF, ("[WscPBCBssTableSort] TmpInfo_5G UUID-E= "));
                                        for(idx=0; idx<16; idx++)
                                                MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF, ("%02x  ", TmpInfo_5G.Uuid[idx]));
                                MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF, ("\n"));
                        } 
                        else
                        {
                                RTMPMoveMemory(&TmpInfo_2G.Uuid[0], &ApUuidBssid[0].Uuid,16);
                                TmpInfo_2G.Band = WSC_RFBAND_24GHZ;

                                MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF, ("[WscPBCBssTableSort] TmpInfo_2G UUID-E= "));
                                        for(idx=0; idx<16; idx++)
                                                MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF, ("%02x  ", TmpInfo_2G.Uuid[idx]));
                                MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF, ("\n"));
                        }
#endif /*CON_WPS Dung_Ru*/
		...
		if (CurOpMode == AP_MODE)
		{
			NdisZeroMemory(pAd->ApCfg.ApCliTab[apcli_idx].CfgSsid, MAX_LEN_OF_SSID);
			NdisMoveMemory(pAd->ApCfg.ApCliTab[apcli_idx].CfgSsid, ApUuidBssid[0].Ssid, ApUuidBssid[0].SsidLen);
			pAd->ApCfg.ApCliTab[apcli_idx].CfgSsidLen = (UCHAR)ApUuidBssid[0].SsidLen;
			pAd->ApCfg.ApCliTab[apcli_idx].MlmeAux.Channel = ApUuidBssid[0].Channel;
		}
#endif /* APCLI_SUPPORT */
    ...
    else
	{
		...
		switch (encyType)
		{
			case WSC_ENCRTYPE_NONE:
				break;
			case WSC_ENCRTYPE_WEP:
				apidx = apidx; /* For compiler Warning */
#ifdef CONFIG_AP_SUPPORT
				if (CurOpMode == AP_MODE)
    ...
    IN  BOOLEAN         bEnrollee)
{
...

	{
		...
		if (pCredential->EncrType != WSC_ENCRTYPE_NONE)
		{
				...
				{
					MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("Unsupport default key index (%d), use key Index 1.\n", WepKeyId));
					pApCliTab->wdev.SecConfig.PairwiseKeyId = 0;
				}
			}
		}
    ...
    IN MLME_QUEUE_ELEM 	   *Elem)
{
	...
	if (CurOpMode == 0xFF)
	{
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_WARN, ("Unkown OpMode (CurOpMode=0x%02x)\n", CurOpMode));
		return;
	}
	else
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("CurOpMode=0x%02x\n", CurOpMode));
...
*/
    {
        ...
        MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_WARN, ("%s - WSC_SINGLE_TRIGGER_APPNAME: %s\n", 
...
#endif
	{
            ...
            {
                if (pAd->dev_idx == 0)
                    send_sig(SIGXFSZ, p, 0);  /* ra0 */
                else
                    send_sig(SIGWINCH, p, 0); /* rai0 */
            }
        }
        rcu_read_unlock();
    }
...
VOID WscWriteConfToDatFile(RTMP_ADAPTER *pAd, UCHAR CurOpMode)
{
	...
	if (CurOpMode == AP_MODE)
	{
		...
		{
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("<----- WscWriteConfToDatFile (wrong apidx = %d)\n", apidx));
			return;
		}
		pWscControl = &pAd->ApCfg.MBSSID[apidx].WscControl;
	...
	{
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("-->1) %s: Error opening file %s\n", __FUNCTION__, fileName));
		return;
	}
	else
	{
		...
		if (cfgData == NULL)
		{
			...
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("CfgData mem alloc fail. (fileLen = %ld)\n", fileLen));
			goto out;
		}
			...
			if ((size_t)(offset - cfgData) < fileLen)
			{
			...
			else if (strncmp(pTempStr, "EncrypType=", strlen("EncrypType=")) == 0)
				{
					...
					if (CurOpMode == AP_MODE)
					{
						...
						{
							if (index == 0)
								snprintf(pTempStr, 512, "%s%s", pTempStr, GetEncryModeStr(pAd->ApCfg.MBSSID[index].wdev.SecConfig.PairwiseCipher));
							else
								snprintf(pTempStr, 512, "%s;%s", pTempStr, GetEncryModeStr(pAd->ApCfg.MBSSID[index].wdev.SecConfig.PairwiseCipher));
						}
					}
    ...
    IN  UCHAR			CurOpMode)
{
	...
	if (CurOpMode == AP_MODE)
	{
		...
		snprintf((RTMP_STRING *) WepKeyName, sizeof(WepKeyName), "Key%dStr1=", pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.SecConfig.PairwiseKeyId+1);
		snprintf((RTMP_STRING *) WepKeyFormatName, sizeof(WepKeyFormatName), "Key%dType=", pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.SecConfig.PairwiseKeyId+1);
	}
#endif /* CONFIG_AP_SUPPORT */
	...
	IN PVOID SystemSpecific3)
{
	...
	pWscControl->WscProfile.ApplyProfileIdx &= 0x7FFF;
	{
		...
		*/
		RfIC = wmode_2_rfic(pMbss->wdev.PhyMode);
		enq_rv = RTEnqueueInternalCmd(pAd, CMDTHREAD_AP_RESTART, &RfIC, sizeof(UCHAR));
...
INT WscApShowPin(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pObj->ioctl_if_type == INT_APCLI) {
		...
		bFromApCli = TRUE;		
		pWscControl = &pAd->ApCfg.ApCliTab[inf_idx].WscControl;
		printk("IF(apcli%d) WPS Information:\n", inf_idx);
		if (pAd->ApCfg.ApCliTab[inf_idx].WscControl.WscEnrolleePinCodeLen == 8)
			printk("Enrollee PinCode(ApCli%d)        %08u\n", 
				inf_idx,
				pAd->ApCfg.ApCliTab[inf_idx].WscControl.WscEnrolleePinCode);
		else
			printk("Enrollee PinCode(ApCli%d)        %04u\n", 
				inf_idx,
				pAd->ApCfg.ApCliTab[inf_idx].WscControl.WscEnrolleePinCode);
		printk("Ap Client WPS Profile Count     = %d\n", pAd->ApCfg.ApCliTab[inf_idx].WscControl.WscProfile.ProfileCnt);
		for (idx_2 = 0; idx_2 < pAd->ApCfg.ApCliTab[inf_idx].WscControl.WscProfile.ProfileCnt ; idx_2++) {
			PWSC_CREDENTIAL pCredential = &pAd->ApCfg.ApCliTab[inf_idx].WscControl.WscProfile.Profile[idx_2];
			printk("Profile[%d]:\n", idx_2);
	...
	{
		bFromApCli = FALSE;
        	pWscControl = &pAd->ApCfg.MBSSID[inf_idx].WscControl;
		printk("IF(ra%d) WPS Information:\n", inf_idx);
#ifdef BB_SOC
		printk("WPS Wsc2MinsTimerRunning(ra%d)        = %d\n", 
			inf_idx, pAd->ApCfg.MBSSID[inf_idx].WscControl.Wsc2MinsTimerRunning);
#else
		/* display pin code */
		if (pAd->ApCfg.MBSSID[inf_idx].WscControl.WscEnrolleePinCodeLen == 8)
			printk("Enrollee PinCode(ra%d)           %08u\n", 
				inf_idx, pAd->ApCfg.MBSSID[inf_idx].WscControl.WscEnrolleePinCode);
		else
	...
	IN  PUCHAR			pMacAddr)
{
	...
	if (pListEntry)
	{
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscDelListEntryByMAC : pMacAddr = %02X:%02X:%02X:%02X:%02X:%02X\n", PRINT_MAC(pMacAddr)));
		delEntryList(pWscEnList, pListEntry);
...
{	
...
#ifdef APCLI_SUPPORT
	if (from_apcli) {
		PWSC_CTRL pWpsCtrl = &pAd->ApCfg.ApCliTab[inf_idx].WscControl;
#ifdef WSC_V2_SUPPORT
		...
		os_alloc_mem(pAd, &pWpsCtrl->pWscRxBuf, MGMT_DMA_BUFFER_SIZE);
		if (pWpsCtrl->pWscRxBuf)
			NdisZeroMemory(pWpsCtrl->pWscRxBuf, MGMT_DMA_BUFFER_SIZE);
		...
		os_alloc_mem(pAd, &pWpsCtrl->pWscTxBuf, MGMT_DMA_BUFFER_SIZE);
		if (pWpsCtrl->pWscTxBuf)
			NdisZeroMemory(pWpsCtrl->pWscTxBuf, MGMT_DMA_BUFFER_SIZE);
...
#endif /* APCLI_SUPPORT */
	{
...
#endif /*HOSTAPD_SUPPORT*/
		{
			...
			//snowpin test
			DebugLevel = 3;
			hex_dump("UUID", &pWscControl->Wsc_Uuid_E[0], UUID_LEN_HEX);
			DebugLevel = 1;
			//snowpin test
