	...
	IN  PWSC_CTRL           pWscControl)
{
	...
	{
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR, ("WscParseEncrSettings --> HMAC not match\n"));
	...
	IN  PWSC_CTRL           pWscControl)
{
	...
	while (PlainLength > 4)
	{
		...
		switch (WscType)
		{
			...
			default:
				MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscProcessCredential --> Unknown IE 0x%04x\n", WscType));
				break;
		}
	...
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscProcessCredential --> %d profile retrieved from credential\n", Cnt));
	return TRUE;
}
	...
	UCHAR *pPSK1)
{
	...
	if (pTempPsk == NULL)
	{
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR, ("%s - pTempPsk alloc failed.", __FUNCTION__));
		return;
	}
	...
	OUT	PWSC_REG_DATA pReg)
{
	...
	RT_DH_PublicKey_Generate (
		...
		if (pWscControl->bTriggerByNFC)
		{
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR, ("Re-generate public key for negative test!\n"));
			...
			case WSC_ID_PUBLIC_KEY:
				...
				if (pWscControl->bTriggerByNFC)
				{
					...
					if (NdisEqualMemory(&HashData[0], &pWscControl->PeerNfcPasswdHash[0], NFC_DEV_PASSWD_HASH_LEN) == FALSE)
					{
						...
						MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR, ("Enrollee public key hash different!\n"));
	...
	OUT	PWSC_REG_DATA 	pReg)
{
	...
	if (pWscControl->bTriggerByNFC && pWscControl->NfcModel == MODEL_HANDOVER)
	{
		...
		if(NULL == IV_DecrData)
		{
			...
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR, ("Allocate IV_DecrData fail @ %s\n", __FUNCTION__));
			return ret;
		}
	}
#endif /* WSC_NFC_SUPPORT */
	...
	while (Length > 4)
	{
		...
		switch (WscType)
		{
			...
			case WSC_ID_PUBLIC_KEY:
				...
				{
					MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("NfcModel(=%d) \n",  pWscControl->NfcModel));
					UCHAR HashData[SHA256_DIGEST_SIZE];
					...
					if (NdisEqualMemory(&HashData[0], &pWscControl->NfcPasswdHash[0], NFC_DEV_PASSWD_HASH_LEN) == FALSE)
					{
						...
						MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR, ("Registrar public key hash different!\n"));
			...
			case WSC_ID_ENCR_SETTINGS:
				...
				if (pWscControl->bTriggerByNFC && pWscControl->NfcModel == MODEL_HANDOVER)
				{
					MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WSC_ID_ENCR_SETTINGS at %s\n", __FUNCTION__));
					if (WscLen <= 16)
						break;
					if (WscLen > 1024)
					{
	...
	if (pWscControl->bTriggerByNFC && bWscEncrSettings && pWscControl->NfcModel == MODEL_HANDOVER)
	{
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("M2 ApEncrSettings len = %d\n ", EncrLen));
