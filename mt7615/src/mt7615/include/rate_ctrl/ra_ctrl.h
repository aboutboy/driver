...
union WTBL_2_DW9 {
    struct {
        ...
        UINT_32 g16:1;
        UINT_32 rsv_9_20:3;
        UINT_32 mpdu_fail_cnt:3;
        UINT_32 mpdu_ok_cnt:3;
        UINT_32 rate_idx:3;
    } field;
    UINT_32 word;
};
...
#define CLIENT_STATUS_TEST_FLAG(_pRaEntry,_F)         \
(((_pRaEntry)->ClientStatusFlags & (_F)) != 0)

#define max(_a, _b) (((_a) > (_b)) ? (_a) : (_b))
#define min(_a, _b) (((_a) < (_b)) ? (_a) : (_b))

/* gl_kal_rom.h */
#define os_move_mem(Destination, Source, Length) kalMemMove(Destination, Source, Length)
#define os_zero_mem(Destination, Length)         kalMemZero(Destination, Length)
#define os_equal_mem(Source1, Source2, Length)   (!kalMemCmp(Source1, Source2, Length))

#define DBG_CAT_RA_DBG_LVL_OFF_PRINT(Fmt)     DBGLOG(RA, ERROR, Fmt)
#define DBG_CAT_RA_DBG_LVL_ERROR_PRINT(Fmt)   DBGLOG(RA, ERROR, Fmt)
#define DBG_CAT_RA_DBG_LVL_WARN_PRINT(Fmt)    DBGLOG(RA, WARN, Fmt)
#define DBG_CAT_RA_DBG_LVL_TRACE_PRINT(Fmt)   DBGLOG(RA, STATE, Fmt)
#define DBG_CAT_RA_DBG_LVL_INFO_PRINT(Fmt)    DBGLOG(RA, INFO, Fmt)
#define DBG_CAT_RA_DBG_LVL_LOUD_PRINT(Fmt)    DBGLOG(RA, LOUD, Fmt)

#define MTWF_LOG(Category, SubCategory, Level, Fmt) Category##_##Level##_PRINT(Fmt)
...
/*******************************************************************************
*                   F U N C T I O N   D E C L A R A T I O N S (FW)
********************************************************************************
*/
#ifdef WIFI_BUILD_RAM
VOID
...
VOID
DynamicTxRateSwitchingEnable(
    VOID
    );

VOID
RateAdjustQuickResponseEnable(
    UINT_8 ucWlanIdx
    );

WLAN_STATUS
RAPowerSavingHandler(
    BOOL fgPwrSavingEnable
    );
...
WLAN_STATUS
raExtCmdGetTxStatistic(
    P_MSDU_INFO_T prMsduInfo
    );

WLAN_STATUS
hemExtEventRaThroughputBurst(
    BOOL fgEnable
    );

WLAN_STATUS
hemExtEventGetTxStatistic(
    P_MSDU_INFO_T prMsduInfo
    );

WLAN_STATUS
hemExtEventMaxAMSDULengthUpdate(
    UINT_8 ucWlanIdx,
    UINT_8 ucAmsduLen
    );
...
VOID
APMlmeDynamicTxRateSwitchingLegacy(
    IN struct _RTMP_ADAPTER *pAd,
    IN UINT i
    );

VOID
APQuickResponeForRateAdaptLegacy(
    IN struct _RTMP_ADAPTER *pAd,
    IN UINT idx
    );
