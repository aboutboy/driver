...
==========================================================================
    Description:
        Enable ATE to read calibrated CR 
        	and write it to flash for Wi-Fi driver.
	...
	IN	PSTRING			arg)
{
	...
	if (value > 0)
	{
		pATEInfo->bSaveFlashEnabled = TRUE;
		DBGPRINT(RT_DEBUG_TRACE,
			("ATE: bSaveFlashEnabled = TRUE , triggered to write calibrated CR to flash  !\n"));
	}
	else
	{
		pATEInfo->bSaveFlashEnabled = FALSE;
		DBGPRINT(RT_DEBUG_TRACE,
			("ATE: bSaveFlashEnabled = FALSE , not triggered to write calibrated CR to flash !\n"));
	}	

	return TRUE;
}

#define	MT7610E_EEPROM_NIC2_OFFSET 0x42

/* 
==========================================================================
    Description:
        Store calibrated CR to flash for Wi-Fi driver
        	to apply when power on.
        
        0: do nothing
        1: store CR, if (bSaveFlashEnabled == TRUE)
        2: dump CR only, not store CR to flash if (bSaveFlashEnabled == TRUE)
        
        Return:
        	TRUE if all parameters are OK, FALSE otherwise
==========================================================================
*/
INT	Set_ATE_CAL_FLASH_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	ATE_INFO *pATEInfo = &(pAd->ate);
	...
	if (pATEInfo->bSaveFlashEnabled == FALSE)
	{
		DBGPRINT_ERR(("ATE: Please set ATEENABLEFLASH=1 first !\n"));
		return FALSE;
	}

	if (value < 3)
	{
		if (MT76x0ATE_SaveFlashCR(pAd, value) == NDIS_STATUS_SUCCESS)
		{
			if (value == 0)
			{
				/* all the CR in flash has been cleared as 0x00 */
				DBGPRINT(RT_DEBUG_TRACE,
					("ATE: Control registers in flash have been cleared as 0x00 !\n"));
			}

			if (value == 1)
			{
				/* CR has been stored in flash */
				/* set valid bit(i.e., EEPROM[0x42] bit 12) to 1 */				
				RT28xx_EEPROM_READ16(pAd, MT7610E_EEPROM_NIC2_OFFSET, e2p_value);
				e2p_value |= (1 << 12);
				RT28xx_EEPROM_WRITE16(pAd, MT7610E_EEPROM_NIC2_OFFSET, e2p_value);
				DBGPRINT(RT_DEBUG_TRACE,
					("ATE: Calibrated control registers have been saved in flash !\n"));
			}
		}
		else
		{
			DBGPRINT_ERR(("ATE: Save calibrated control registers in flash failed !\n"));			
			return FALSE;
		}
	}
	else
	{
		DBGPRINT(RT_DEBUG_TRACE,
			("ATE: Not saving calibrated control registers in flash !\n"));			
	}
	...
	IN PVOID SystemSpecific3)
{
	...
	if (ATE_ON(pAd))
	{
		...
		if ((IS_MT76x0(pAd) && pAd->chipCap.bDoTemperatureSensor) &&
			(pATEInfo->PeriodicRound % (ATE_TASK_EXEC_MULTIPLE * 5) == 0))
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("ATE_VGA_EXT: MT76x0_TempSensor --> %d\n", 
			...
			if (CurTempSensorState != pAd->chipCap.LastTempSensorState)
			{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("ATE_VGA_EXT: Change in new TS state %d, BW:%d\n", tsIdx, bwIdx));
				DBGPRINT(RT_DEBUG_TRACE, ("ATE_VGA_EXT: Target Gain ==> %02X\n", eLNAgain));

				/* bypass Dynamic VGA for Verification */
				RTMP_BBP_IO_WRITE32(pAd, AGC1_R8, (vga_ext_val[tsIdx][bwIdx]&(~0x0000FF00))|(eLNAgain << 8));
			...
			{
				DBGPRINT(RT_DEBUG_INFO, ("ATE_VGA_EXT: Keep in the Last TS state %d, BW:%d\n",
