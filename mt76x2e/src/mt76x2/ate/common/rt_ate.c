	...
	IN PRTMP_ADAPTER pAd)
{
	...
	ed_monitor_exit(pAd);
#endif
	//STOP secondary EDCCA -mac 1608 bit 1 in ATE mode
	...
	//WCNAE00088181 - resume normal mode in ATE to avoid TX EVM Fail in DFS Channel
	pAd->Dot11_H.RDMode = RD_NORMAL_MODE; 
	
#ifdef SINGLE_SKU_V2
	pATEInfo->BBPCurrentBW_Backup = pAd->CommonCfg.BBPCurrentBW;
...
if ( IS_MT76x2(pAd) )
{
	/* quick TSSI compensation for MPS */
	if ( (pATEInfo->Mode == ATE_TXFRAME) && (pATEInfo->bAutoTxAlc == TRUE)) //&& (pATEInfo->MPS_Start))
	{
		UCHAR i=0;
		for(i=0;i<5;i++)
		{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("@@@@ MPS TSSI compensated %d times\n", i));
	}
}
...
#endif /* RT6352 */
		if(pATEInfo->PreviousChannel != pATEInfo->Channel)
		{
	...
	IN	PSTRING			arg)
{
	...
	/* If the chain configuration changes, force reconfiguration of the channel */
	if( value != pATEInfo->TxAntennaSel )
	{
	...
	IN	PSTRING			arg)
{
	...
	/* If the chain configuration changes, force reconfiguration of the channel */
	if( value != pATEInfo->RxAntennaSel )
	{
	...
	if (ATE_ON(pAd))
	{
		...
		if ( IS_MT76x2(pAd) )
		{
			if ( (pATEInfo->Mode == ATE_TXFRAME) && (pATEInfo->bAutoTxAlc == TRUE) )			{
				...
				//DBGPRINT(RT_DEBUG_TRACE, ("@@@@ ATE TSSI compensated %d times\n", i));
				pAd->ate.bSendTSSICmdOnly = FALSE;
				ATEAsicAdjustTxPower(pAd);
			}
		}
#endif /* MT76x2 */
